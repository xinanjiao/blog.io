...
哈希的poj经典题目挺多，今天接触了两道题，关于数字哈希的，原来背了一个关于处理一个数的hash模板，但在这两个题都用不到，但我也学会了储存连续的数的状态hash，**用vector a[maxn],maxn存储他们的和，但总和要除余hash表长，vector保存该一串数字的状态**，相比其他形式的hash，简单易懂还很好理解。
